...
PRIMARY_CP_NAME="${PRIMARY_CP_NAME:-cp1}"
...
remote_state() {
  vagrant_cmd ssh "${PRIMARY_CP_NAME}" -c "systemctl is-active k3s || true" 2>/dev/null | tr -d '\r'
}

dump_k3s_diagnostics() {
  echo "---- ${PRIMARY_CP_NAME}: systemctl status k3s ----" >&2
  vagrant_cmd ssh "${PRIMARY_CP_NAME}" -c "sudo systemctl status --no-pager --full k3s || true" >&2 || true
  echo "---- ${PRIMARY_CP_NAME}: journalctl -u k3s (last 120 lines) ----" >&2
  vagrant_cmd ssh "${PRIMARY_CP_NAME}" -c "sudo journalctl -u k3s --no-pager -n 120 || true" >&2 || true
}
...
while true; do
  ...
  if [[ "${k3s_state}" == "failed" ]]; then
    echo "primary control-plane k3s service is in failed state; aborting API wait." >&2
    dump_k3s_diagnostics
    exit 1
  fi

  if vagrant_cmd ssh "${PRIMARY_CP_NAME}" -c "curl -sk https://127.0.0.1:6443/readyz >/dev/null"; then
    cp1_local_ready=1
  fi

  if vagrant_cmd ssh "${PRIMARY_CP_NAME}" -c "curl -sk https://${CP1_IP}:6443/readyz >/dev/null"; then
    cp1_advertised_ready=1
  fi

  if [[ "${KUBE_API_LB_ENABLED}" == "true" && "${KUBE_CP_COUNT}" -gt 1 ]]; then
    lb_required=1
    if vagrant_cmd ssh "${PRIMARY_CP_NAME}" -c "curl -sk https://${KUBE_API_LB_IP}:6443/readyz >/dev/null"; then
      lb_ready=1
    fi
  fi
  ...
  if (( waited == 0 || waited % report_interval == 0 )); then
    log_wait_progress "Waiting for k3s API readiness (k3s-state=${k3s_state:-unknown}, primary-local=${cp1_local_ready}, primary-advertised=${cp1_advertised_ready}, lb-required=${lb_required}, lb=${lb_ready})" "${waited}" "${MAX_WAIT_SECONDS}" "${report_interval}"
  fi

  if [[ "${waited}" -ge "${MAX_WAIT_SECONDS}" ]]; then
    echo "Timed out waiting for k3s API readiness (primary=${CP1_IP}, lb=${KUBE_API_LB_IP})" >&2
    dump_k3s_diagnostics
    exit 1
  fi
